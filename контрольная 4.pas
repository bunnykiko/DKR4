...
begin
  ...
  setwindowtitle('График функции: 1*x^3+(-2)*x^2+(3)*x+(5)');
  setfontcolor(clMaroon);
  setfontsize(10);
  textout(30, 30, '1*power(x1, 3)+(-2)*power(x1, 2)+(3)*x1+(5)');
  setpencolor(clBlack);
  setpenwidth(1);
  line(80, 400, 720, 400);
  line(400, 30, 400, 520);
  ...
  for i:= -300 to 300 do
    begin
      x1:=i/n;
      y1:=1*power(x1, 3)+(-2)*power(x1, 2)+(3)*x1+(5);
      ...
      if (trunc(400+x1*n)=m) and (m<b) then
          begin
            ...
            x2:=(i+c)/n;
            y2:=1*power(x1, 3)+(-2)*power(x1, 2)+(3)*x1+(5);
